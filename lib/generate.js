...
- Testimonials: only when extra pages were purchased and reviews or entered testimonials exist; white rounded-2xl cards, teal quote mark, teal stars when a rating exists, author row with initial avatar, slate quote text. Omit the section otherwise.
...
Extra-pages requested: ${wantsPaidExtraPages(clinic.extraPages) ? "yes — include a testimonials section (and other extra pages only if that copy exists)" : "no — do not add testimonials, facilities, team, or doctor-listing pages"}
...
- Testimonials: add a testimonials section only if Extra pages is the paid Yes option AND manual testimonials or Google Maps reviews exist. Style cards as described in the visual guide. Use that copy only. Do not invent reviews. Omit testimonials when Extra pages was not purchased.
...
function buildPagePrompt(plannedPrompt, images, serviceItems = [], clinic = {}, reviewData = { reviews: [] }) {
  ...
  const wantsTestimonials = wantsPaidExtraPages(clinic.extraPages);
  ...
  const reviewGuide = !wantsTestimonials
    ? "The Extra pages column did not select the paid Yes option. Do not add a testimonials section, testimonials nav link, facilities page, team page, or doctor listing."
    : reviewData.reviews?.length || hasManual
      ? `Add a testimonials section using this copy only. Do not invent extra reviews.\nManual testimonials:\n${manualText}\n\nGoogle Maps reviews:\n${formatReviewsForPrompt(reviewData)}`
      : "Extra pages were purchased, but no testimonials were entered and no Google Maps reviews were fetched. Do not invent a testimonials section.";
...
function embedImagesInHtml(html, images, serviceItems = [], wantsGallery = true) {
  let result = html;
...
.contact-btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;padding:12px 20px;border-radius:12px;background:#0f766e;color:#fff;text-decoration:none;font-weight:700}
...
.testimonials-kicker{display:inline-flex;margin:0 0 10px;padding:6px 12px;border-radius:999px;background:#ccfbf1;color:#0f766e;font-size:.72rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase}
.testimonials-head h2{margin:0;font-size:2rem;color:#0f172a}
.testimonials-head .muted{margin:8px 0 0;color:#64748b}
.testimonials-more{color:#0f766e;font-weight:700;text-decoration:none}
.testimonials-grid{display:grid;gap:24px;grid-template-columns:repeat(auto-fit,minmax(260px,1fr))}
.testimonial-card{position:relative;padding:32px 28px 28px;border:1px solid #e2e8f0;border-radius:16px;background:#fff;box-shadow:0 10px 30px rgba(15,23,42,.05)}
.testimonial-quote{position:absolute;top:16px;right:20px;color:#99f6e4;font-size:3rem;line-height:1;font-family:Georgia,serif}
.testimonial-stars{margin:0 0 12px;color:#0d9488;letter-spacing:2px}
...
.testimonial-avatar{width:40px;height:40px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;background:#0f766e;color:#ecfdf5;font-weight:800;font-size:.85rem}
...
function testimonialsSectionHtml(reviewData) {
  ...
  return `<section id="testimonials"><div class="testimonials-wrap"><div class="testimonials-head"><div><p class="testimonials-kicker">Patient stories</p><h2>Testimonials</h2>${ratingLine}</div>${moreLink}</div><div class="testimonials-grid">${cards}</div></div></section>`;
}
...
function applyPageFixes(html, clinic, reviewData = { reviews: [] }) {
  ...
  result = ensureTestimonials(result, reviewData, wantsPaidExtraPages(clinic.extraPages));
...
export async function generateClinicPage(clinic, onProgress = () => {}) {
  ...
  return {
    html: applyPageFixes(embedImagesInHtml(html, images, serviceItems, wantsGallery), clinic, {
      ...reviewData,
