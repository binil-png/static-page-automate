...
import { paletteFromLogoImage } from "./logoPalette.js";
...
export async function generateClinicPage(clinic, onProgress = () => {}) {
  ...
  onProgress({ percent: 8, step: `Using all reference layouts. Primary: ${reference.id}...` });
  ...
  const logoImage = images.find((image) => image.role === "logo" && image.source !== "generated" && image.dataUri);
  const logoPalette = logoImage ? paletteFromLogoImage(logoImage) : null;
  const palette = logoPalette || pickColorPalette();
  const wantsGallery = wantsReadyPhotoGallery(clinic.photoGallery);
  if (folder.exists) {
    onProgress({
      percent: 18,
  ...
  if (needsGeneratedLogo(clinic.logo) && !images.some((image) => image.role === "logo")) {
    onProgress({ percent: 19, step: "No clinic logo or icon file was found. Header will use the clinic name only..." });
  }
  if (logoPalette) {
    console.log(`Using logo colors ${palette.deep} (${palette.family}). Primary layout: ${reference.id}.`);
    onProgress({ percent: 20, step: `Theme colors taken from the uploaded logo (${palette.deep}).` });
  } else {
    console.log(`Using all reference pages (${referenceNames}). Primary layout: ${reference.id}. Palette: ${palette.family}.`);
    onProgress({ percent: 20, step: `Using all reference layouts. Primary: ${reference.id}, colors: ${palette.family}...` });
  }
  const serviceItems = parseServiceItems(clinic.services);
