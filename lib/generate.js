...
- Testimonials: if Google Maps reviews were fetched, add a testimonials section that uses those reviews only. Include author, star rating, and review text. Do not invent reviews.
...
function buildPagePrompt(plannedPrompt, images, serviceItems = [], clinic = {}, reviewData = { reviews: [] }) {
  ...
  const reviewGuide = reviewData.reviews?.length
    ? `Add a testimonials section using only these Google Maps reviews:\n${formatReviewsForPrompt(reviewData)}`
    : "No Google Maps reviews were fetched. Do not invent a testimonials section.";
...
.social-link{display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:50%;background:#1f5c4d;color:#fff;text-decoration:none}
.social-link span{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0)}
.booking-btn{display:inline-flex;align-items:center;justify-content:center;padding:12px 18px;border-radius:999px;background:#1f5c4d;color:#fff;text-decoration:none;font-weight:700}
.contact-email,.contact-item{margin:8px 0}
.contact-label{display:block;font-size:.8rem;letter-spacing:.05em;text-transform:uppercase;opacity:.7}
#testimonials{padding:48px 5%}
.testimonials-head{display:flex;justify-content:space-between;gap:16px;align-items:end;flex-wrap:wrap;margin-bottom:24px}
.testimonials-grid{display:grid;gap:16px;grid-template-columns:repeat(auto-fit,minmax(240px,1fr))}
.testimonial-card{padding:20px;border:1px solid #d9d1c3;border-radius:16px;background:#fff}
.testimonial-stars{margin:0 0 8px;color:#1f5c4d;letter-spacing:2px}
.testimonial-card blockquote{margin:0 0 12px}
.testimonial-author{margin:0;font-weight:700}
.testimonial-time{margin:4px 0 0;opacity:.7;font-size:.9rem}
</style>`;
}
...
function testimonialsSectionHtml(reviewData) {
  const cards = reviewData.reviews.map((review) => (
    `<article class="testimonial-card"><p class="testimonial-stars" aria-label="${review.rating} out of 5">${starRating(review.rating)}</p><blockquote>${escapeHtml(review.text)}</blockquote><p class="testimonial-author">${escapeHtml(review.author)}</p>${review.relativeTime ? `<p class="testimonial-time">${escapeHtml(review.relativeTime)}</p>` : ""}</article>`
  )).join("");
...
export async function generateClinicPage(clinic, onProgress = () => {}) {
  ...
  try {
    plannerResponse = await generateContentWithRetry([
      {
        role: "user",
        parts: [createPartFromText(buildPlannerPrompt(clinic, images, reviewData))]
      }
    ], "website prompt");
  } catch (error) {
  ...
  const pageParts = [createPartFromText(buildPagePrompt(plannedPrompt, images, serviceItems, clinic, reviewData))];
  for (const image of images) {
  ...
  return {
    html: applyPageFixes(embedImagesInHtml(html, images, serviceItems), clinic, reviewData),
