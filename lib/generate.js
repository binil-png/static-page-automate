...
- Contact: exactly one section with id="contact". Put address, phone, email, timing, map, and the form in that same section. Do not add a second Get in Touch, Reach Us, or Contact Us block.
...
Photo gallery requested: ${wantsReadyPhotoGallery(clinic.photoGallery) ? "yes — add a Gallery menu item and show ALL Drive photos in a gallery section. Also use photos in hero, about, and services where they fit." : "no — do not add a gallery section or Gallery menu link. Still use Drive photos in hero, about, or service tiles."}
...
- Use every attached Drive photo somewhere on the page (hero, about, matching service tiles, or other fitting sections).
...
function ensureEmailInContact(html, email) {
  ...
  const contactOpen = firstContactOpenTag(html);

  if (contactOpen) {
    return html.replace(contactOpen, `${contactOpen}\n${row}`);
  }
