...
function clinicEmail(clinic = {}) {
  return String(clinic.email || "").trim().toLowerCase();
}

export function clinicFilesFolderName(email, date = new Date()) {
  const day = typeof date === "string" ? date : date.toISOString().slice(0, 10);
  return `Clinic files - ${String(email || "").trim()} - ${day}`;
}

function isClinicFilesFolderName(name, email) {
  const folderName = String(name || "").trim().toLowerCase();
  const address = String(email || "").trim().toLowerCase();
  if (!address || !folderName.startsWith("clinic files - ")) {
    return false;
  }
  return folderName.includes(address);
}

function folderNameDate(name) {
  const match = String(name || "").match(/(\d{4}-\d{2}-\d{2})\s*$/);
  return match ? match[1] : "";
}

function folderMatchesClinic(name, needles, clinic = {}) {
  if (isClinicFilesFolderName(name, clinicEmail(clinic))) {
    return true;
  }
...
async function listCandidateFolders(drive, parentFolderId, clinic = {}) {
  ...
  const terms = [];
  const email = clinicEmail(clinic);
  if (email) {
    terms.push(`Clinic files - ${email}`);
    terms.push(email);
  }
...
export async function findClinicFolder(clinic) {
  ...
  if (!clinicEmail(clinic) && !needles.length) {
  ...
  try {
    ...
    const email = clinicEmail(clinic);
    const named = folders.filter((folder) => isClinicFilesFolderName(folder.name, email));
    const matched = named.length
      ? named
      : folders.filter((folder) => folderMatchesClinic(folder.name, needles, clinic));
    const match = matched.sort((left, right) => {
      const byDate = folderNameDate(right.name).localeCompare(folderNameDate(left.name));
      if (byDate) {
        return byDate;
      }
...
export async function createClinicFolder(clinic) {
  ...
  const email = clinicEmail(clinic);
  const name = email
    ? clinicFilesFolderName(email, date)
    : `Clinic files - ${clinic.clinicName || "clinic"} - ${date}`;
