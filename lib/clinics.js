...
export function needsGeneratedLogo(value) {
  const text = normalizeChoice(value);
  return text.includes("i need a logo for my clinic")
    || (text.includes("starting from inr 5000") && text.includes("2 concepts"));
}

export function wantsReadyPhotoGallery(value) {
  const text = normalizeChoice(value);
  return text.includes("photos are ready with me")
    || (text.includes("500 extra") && text.includes("1000 pixels"));
}

export function wantsPaidExtraPages(value) {
  const text = normalizeChoice(value);
  return text.includes("400 per page extra")
    || (text.includes("rs 400") && text.includes("10 pages"))
    || (text.includes("400 per page") && text.includes("twenty lines"));
}
...
function rowToClinic(row, index) {
  ...
  clinic.needsGeneratedLogo = needsGeneratedLogo(clinic.logo);
  clinic.wantsPhotoGallery = wantsReadyPhotoGallery(clinic.photoGallery);
  clinic.wantsExtraPages = wantsPaidExtraPages(clinic.extraPages);
  return clinic;
}
