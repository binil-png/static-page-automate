...
export async function saveSetupConfig(nextValues) {
  ...
  if (Object.prototype.hasOwnProperty.call(nextValues, "openaiApiKey")) {
    const key = String(nextValues.openaiApiKey || "").trim();
    if (key) {
...
export async function getOpenAiApiKey() {
  const setup = await loadSetupConfig();
  return String(process.env.OPENAI_API_KEY || setup.openaiApiKey || "").trim();
}
