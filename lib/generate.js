...
function fileLooksLikeLogo(name) {
  ...
  return base === "logo" || /^logo\s*\(\d+\)$/.test(base);
}
...
function logoPriority(name) {
  return fileLooksLikeLogo(name) ? 0 : 1;
}
...
- Include a logo in the header only if a resource role is logo. A logo is used only when a Drive image file is named logo (for example logo.png).
...
- ${hasLogo ? "Use the attached logo file in the header brand area only." : "No Drive image named logo was provided. Do not add a logo image."}
...
export async function generateClinicPage(clinic) {
