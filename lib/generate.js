...
Photo gallery requested: ${wantsReadyPhotoGallery(clinic.photoGallery) ? "yes — include a photo gallery using the clinic photos" : "no — do not add a paid gallery page. Still use every attached Drive photo in hero, about, or service tiles."}
...
function stripExtraServiceVisuals(cardHtml) {
  ...
  eachServiceVisual(cardHtml, (visual) => {
    if (isServiceCardVisual(visual) && !/service-tile-icon/i.test(visual)) {
      hasPreferred = true;
    }
    return visual;
  });
  ...
  const result = eachServiceVisual(cardHtml, (visual) => {
    ...
    if (hasPreferred && /service-tile-icon/i.test(visual)) {
      return "";
    }
    if (kept) {
      return "";
  ...
  return result.replace(/<div\b[^>]*(?:w-1[2-6]|h-1[2-6]|icon)[^>]*>\s*<\/div>/gi, "");
}
...
function ensureServiceTileVisuals(html, serviceItems, images, palette = COLOR_PALETTES[0]) {
  ...
  for (const service of serviceItems) {
    ...
    const photo = images.find((image) => image.matchedService === service)
      || images.find((image) => matchingServiceName(image.originalName, [service]));
    if (photo?.dataUri) {
      const windowStart = serviceCardStart(result, match.index);
      const windowEnd = match.index + match[0].length;
      const visual = `<img class="service-tile-image" src="${photo.dataUri}" alt="${escapeHtml(service)}">`;
      const cleaned = replaceServiceCardVisual(result.slice(windowStart, windowEnd), visual);
      result = `${result.slice(0, windowStart)}${cleaned}${result.slice(windowEnd)}`;
      continue;
    }

    if (headingAlreadyHasVisual(result, match.index)) {
      continue;
    }
    result = result.replace(pattern, `${serviceIconSvg(service, palette)}<$1$2>$3</$1>`);
  }
...
function embedImagesInHtml(html, images, serviceItems = [], wantsGallery = true, palette = COLOR_PALETTES[0]) {
  ...
  const unused = images.filter((image) => image.source !== "generated" && !result.includes(image.dataUri));
  if (!unused.length) {
    return result;
  }

  const photos = unused.map((image) => (
    `<img src="${image.dataUri}" alt="${escapeHtml(image.originalName || image.role)}" class="clinic-drive-photo">`
  )).join("");
  const sectionId = wantsGallery ? "gallery" : "photos";
  const title = wantsGallery ? "Photo gallery" : "Clinic photos";
  const block = `<section id="${sectionId}" class="clinic-drive-photos"><div class="clinic-drive-photos-wrap"><p class="clinic-drive-photos-kicker">From the clinic</p><h2>${title}</h2><div class="clinic-drive-photos-grid">${photos}</div></div></section>`;

  if (/<section[^>]*id=["']contact["']/i.test(result)) {
    return result.replace(/<section[^>]*id=["']contact["']/i, `${block}\n<section id="contact"`);
  }
  if (/<footer/i.test(result)) {
    return result.replace(/<footer/i, `${block}\n<footer`);
  }
